...
export async function PUT(request, { params }) {
  try {
    ...
    const body = await request.json();
    const { name, description, duration, goal, schedule, exercises, price, discount, discount_percentage } = body;
    ...
    if (schedule !== undefined) update.schedule = Array.isArray(schedule) ? schedule : schedule;
    if (exercises !== undefined) update.exercises = Array.isArray(exercises) ? exercises : exercises;
    if (price !== undefined) update.price = typeof price === "number" ? price : Number(price || 0);
    if (discount !== undefined) update.discount = !!discount;
    if (discount_percentage !== undefined)
      update.discount_percentage =
